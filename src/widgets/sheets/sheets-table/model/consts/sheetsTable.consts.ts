import type { ISheetsTableSchema } from '../types/sheetsTable.interface';

export const initialSheetsTableState: ISheetsTableSchema = {
  sheetsTable: {
    id: '',
    cols: [],
    rows: [],
    cells: [],
  },
  selectCells: [],
};

export const CODES = {
  A: 65,
  Z: 90,
};

export const CHARS_MAX_LENGTH = 25;

export const MIN_ROW_HEIGHT = 10;
export const MIN_COL_WIDTH = 30;

export const ROWS_COUNT = 40;
export const COLS_COUNT = 30;

export const RESIZE_MARGIN = 3;
export const ROW_MIN_HEIGHT = 16;
export const COL_MIN_WIDTH = 40;

export const DEFAULT_WIDTH = 120;
export const DEFAULT_HEIGHT = 25;
